...
@section('content')
        ...
        <div class="row">
            <div class="table-responsive">
                <table class="table table-sm table-bordered">
                    ...
                    <tbody>
                        @foreach ($pacients1 as $n => $pacient)
    ...
    <div class="modal right fade" id="myModal2" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <button type="button" id="reset" class="btn btn-success btn-sm mb-3 w-100 text-white">Сбросить все
                        фильтры</button>

                    <form action="{{ Route('PacientsPage') }}" id="search" method="get">
                        <label for="dr122">Поиск по ФИО</label>
                        <input type="search" placeholder="Поиск по ФИО" class="form-control form-control-sm" id="dr122"
                        ...
                        <input type="text" class="form-control form-control-sm dateranger" id="dr1" name="birthday"
                            placeholder="Дата рождения"
                            value="{{ isset($_GET['birthday']) ? $_GET['birthday'] : '' }}" />
                        <div class="row">
                            <div class="col-md-6"><label for="dr2" class="mt-2">Участок</label>
                                <select class="form-select form-select-sm" id="dr2" name="uchastok_id">
                            ...
                            <div class="col-md-6"><label for="dr3" class="mt-2">Роддом</label>
                                <select class="form-select form-select-sm" id="dr3" name="roddom_id">
                        ...
                        <div class="row">
                            <div class="col-md-6"><label for="dr4" class="mt-2">Пол</label>
                                <select class="form-select form-select-sm" id="dr4" name="pol">
                            ...
                            <div class="col-md-6"><label for="dr7" class="mt-2">Скриннинг</label>
                                <select class="form-select form-select-sm" id="dr7" name="skrinning">
                                    <option value="">-</option>
                                    <option value="roddom"
                                        {{ isset($_GET['skrinning']) ? ($_GET['skrinning'] == 'roddom' ? 'selected' : '') : '' }}>
                                        Роддом
                                    </option>
                                    <option value="policlinic"
                                        {{ isset($_GET['skrinning']) ? ($_GET['skrinning'] == 'policlinic' ? 'selected' : '') : '' }}>
                                        Поликлиника
                                    </option>
                                </select>
                            </div>
                        ...
                        <select class="form-select form-select-sm" id="dr5" name="bolezn">
                        ...
                        <input type="text" class="form-control form-control-sm dateranger" id="dr6" name="date_add"
                            placeholder="Период" value="{{ isset($_GET['date_add']) ? $_GET['date_add'] : '' }}" />

                        <div class="row">
                            <div class="col-md-6"><label for="dr8" class="mt-2">ВИЧ</label>
                                <select class="form-select form-select-sm" id="dr8" name="vich">
                                    <option value="">Нет
                                    </option>
                                    <option value="1"
                                        {{ isset($_GET['vich']) ? ($_GET['vich'] == 1 ? 'selected' : '') : '' }}>Да
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6"><label for="dr9" class="mt-2">Гепатит</label>
                                <select class="form-select form-select-sm" id="dr9" name="gepatit">
                                    <option value="">Нет
                                    </option>
                                    <option value="1"
                                        {{ isset($_GET['gepatit']) ? ($_GET['gepatit'] == 1 ? 'selected' : '') : '' }}>Да
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6"><label for="dr10" class="mt-2">Рецепиент</label>
                                <select class="form-select form-select-sm" id="dr10" name="recepient">
                                    <option value="">Нет
                                    </option>
                                    <option value="1"
                                        {{ isset($_GET['recepient']) ? ($_GET['recepient'] == 1 ? 'selected' : '') : '' }}>
                                        Да
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6"><label for="dr11" class="mt-2">СВС</label>
                                <select class="form-select form-select-sm" id="dr11" name="gruppasvs">
                                    <option value="">Нет
                                    </option>
                                    <option value="1"
                                        {{ isset($_GET['gruppasvs']) ? ($_GET['gruppasvs'] == 1 ? 'selected' : '') : '' }}>
                                        Да
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-sm" id="dr11" name="ves">
                                    <option value=""></option>
                                    <option value="1"
                                        {{ isset($_GET['ves']) ? ($_GET['ves'] == 1 ? 'selected' : '') : '' }}>НМТ
                                    </option>
                                    <option value="2"
                                        {{ isset($_GET['ves']) ? ($_GET['ves'] == 2 ? 'selected' : '') : '' }}>ОНМТ
                                    </option>
                                    <option value="3"
                                        {{ isset($_GET['ves']) ? ($_GET['ves'] == 3 ? 'selected' : '') : '' }}>ЭНМТ
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="dr12" class="mt-2">Прививки</label>
                                <select class="form-select form-select-sm" name="vacine" id="dr12">
                                    <option value=""></option>
                                    <option value="1"
                                        {{ isset($_GET['vacine']) ? ($_GET['vacine'] == 1 ? 'selected' : '') : '' }}>Без
                                        прививок
                                    </option>
                                    <option value="2"
                                        {{ isset($_GET['vacine']) ? ($_GET['vacine'] == 2 ? 'selected' : '') : '' }}>Без
                                        БЦЖ-М
                                    </option>
                                    <option value="3"
                                        {{ isset($_GET['vacine']) ? ($_GET['vacine'] == 3 ? 'selected' : '') : '' }}>Без
                                        Гепатита B
                                    </option>
                                    <option value="4"
                                        {{ isset($_GET['vacine']) ? ($_GET['vacine'] == 4 ? 'selected' : '') : '' }}>Все
                                        прививки
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-12">
                                <label for="dr13" class="mt-2">Срок гестации</label>
                                <select class="form-select form-select-sm" name="gestaci" id="dr13">
                                    <option value=""></option>
                                    <option value="1"
                                    ...
                                    <option value="5"
                                        {{ isset($_GET['gestaci']) ? ($_GET['gestaci'] == 5 ? 'selected' : '') : '' }}>
                                        Умеренно недоношенные (32-33)
                                    </option>
                                    <option value="6"
                                        {{ isset($_GET['gestaci']) ? ($_GET['gestaci'] == 6 ? 'selected' : '') : '' }}>
                                        Очень недоношенные (28-31)
                                    </option>
                                    <option value="7"
                                        {{ isset($_GET['gestaci']) ? ($_GET['gestaci'] == 7 ? 'selected' : '') : '' }}>
                                        Крайне недоношенные (<28) </option>
                                </select>
                            </div>
                        </div>
                        <button type="submit" class="btn mt-3 btn-success w-100">Фильтровать</button>
